    ...
    def _prepare_invoice(self):
        ...
        if self.partner_id.vat == False:
            ...
            if self.partner_id.is_company == True:
                ...
                if len(self.partner_id.vat)!=12:
                    raise ValidationError("El RFC %s no tiene la logitud de 12 caracteres para personas Morales que establece el sat" % (self.partner_id.vat))
                else:    
                    patron_rfc = re.compile(r'^([A-ZÑ\x26]{3}([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1]))((-)?([A-Z\d]{3}))?$')
                    if not patron_rfc.search(self.partner_id.vat):
                        msg = "Formato RFC de Persona Moral Incorrecto"
                        raise ValidationError(msg)
            else:
                #Valida el RFC en base al patron de una Persona Fisica
                if len(self.partner_id.vat)!=13:
                    raise ValidationError("El RFC %s no tiene la logitud de 13 caracteres para personas Fisicas que establece el sat" % (self.partner_id.vat))
                else:
        ...
        invoice_vals = {
            ...
            #'codigo_postal_id': self.partner_id.id,
            ...
            'rfc_cliente_factura': (self.partner_id.vat).encode('utf-8'),
